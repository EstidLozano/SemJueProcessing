...
class KeysTracker {
  ...
  KeysTracker(int[] keys, boolean coded) {
    this.keys = keys;
    this.coded = coded;
    
    this.keyStates = new boolean[keys.length];
  }
  
  void keyPressed() {
    if (coded == (key == CODED)) {
      setKeyPressed(coded ? keyCode : key, true);
    }
  }

  void keyReleased() {
    if (coded == (key == CODED)) {
      setKeyPressed(coded ? keyCode : key, false);
    }
  }
  
  boolean isPressed(int k) {
    k = getKeyIndex(k);
    return k != -1 && keyStates[k];
  }
  ...
  private void setKeyPressed(int k, boolean state) {
    k = getKeyIndex(k);
    if (k == -1) return;
    keyStates[k] = state;
  }
